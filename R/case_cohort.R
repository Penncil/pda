...
prepare_case_cohort <- function(data_list, control, full_cohort_size) {
  ...
  for (k in 1:K) {
    ...
    if (surv_cols == 3) {
      covariate_list[[k]] <- as.matrix(data_list[[k]][, -c(1:4)])
    } else if (surv_cols == 2) {
    ...
    if (failure_num[k] == 0) {
      ...
      for (j in 1:failure_num[k]) {
        my_risk_set1 <- which(
          (data_list[[k]]$subcohort == 1) &
            (data_list[[k]]$time >= failure_times[[k]][j]) &
            (data_list[[k]]$time_in <= failure_times[[k]][j])
        )
        risk_size <- risk_size + length(my_risk_set1)
        if (method == "Prentice") {
          my_weight1 <- rep(1, length(my_risk_set1))
          if (data_list[[k]]$subcohort[
            which(data_list[[k]]$time == failure_times[[k]][j])
          ] == 0
          ) {
