...
ODACH_CC.initialize <- function(ipdata, control, config) {
  ...
  surv_cols <- length(as.list(call_obj)) - 1L # -1L for the function name
  n_skip_cols <- surv_cols + 1L # +1L for the subcohort column
  px <- ncol(ipdata) - n_skip_cols

  # handle data degeneration (e.g. missing categories in some site). This could be in pda()?
  if (surv_cols == 3) {
    col_deg <- apply(ipdata[, -c(1:4)], 2, var) == 0 # degenerated X columns...
  } else if (surv_cols == 2) {
    col_deg <- apply(ipdata[, -c(1:3)], 2, var) == 0 # degenerated X columns...
  }

  ipdata_i <- ipdata[, -(which(col_deg) + n_skip_cols), with = F]
  ...
  if (surv_cols == 3) {
    times <- sort(unique(c(ipdata$time_in, ipdata$time)))
...
ODACH_CC.derive <- function(ipdata, control, config) {
  ...
  full_cohort_size <- control$full_cohort_size[control$sites == config$site_id]
  cc_prep <- prepare_case_cohort(list(ipdata), control, full_cohort_size)
  ...
  logL_D1 <- rcpp_cc_grad_plk(
    beta = bbar, site_num = 1,
...
ODACH_CC.estimate <- function(ipdata, control, config) {
  ...
  logL_local <- function(beta) {
    rcpp_cc_log_plk(beta,
      site_num = 1,
      covariate_list = cc_prep$covariate_list,
      failure_position = cc_prep$failure_position,
      failure_num = cc_prep$failure_num,
  ...
  logL_local_D1 <- function(beta) {
    rcpp_cc_grad_plk(beta,
      site_num = 1,
      covariate_list = cc_prep$covariate_list,
      failure_position = cc_prep$failure_position,
      failure_num = cc_prep$failure_num,
  ...
  logL_local_D2 <- function(beta) {
    rcpp_cc_hess_plk(beta,
      site_num = 1,
      covariate_list = cc_prep$covariate_list,
      failure_position = cc_prep$failure_position,
      failure_num = cc_prep$failure_num,
