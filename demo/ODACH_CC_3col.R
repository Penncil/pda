...
print(nrow(odach_cc))
set.seed(0)
time_in <- runif(nrow(odach_cc), 40, 45)
time_out <- time_in + runif(nrow(odach_cc), 2, 5)
odach_cc$time_in <- time_in
odach_cc$time_out <- time_out
odach_cc$ID <- seq(1,nrow(odach_cc))

print(head(odach_cc))
data_split <- split(odach_cc, odach_cc$site)

# ## cch stratified by site, using self-written cch_pooled()
# fit.pool <- pda::cch_pooled(Surv(time, status) ~ X1+`Group (A,B,C)`+Category, data=odach_cc,  
...
data_structure <- list(coef = NA, standard_error = NA, pvalue = NA)
beta_pooled <- data_structure

formulcox <- as.formula(
                "Surv(time_in, time_out, status) ~ X1 + Category + cluster(ID)"
                )
fit_pooled <- survival::coxph(formulcox, data = odach_cc)
beta_pooled$coef <- summary(fit_pooled)$coef[, "coef"]
beta_pooled$standard_error <- summary(fit_pooled)$coef[1, "robust se"]
beta_pooled$pvalue <- summary(fit_pooled)$coef[1, "Pr(>|z|)"]
...
control <- list(project_name = 'ODACH case-cohort toy example',
                ...
                outcome = "Surv(time_in, time_out, status)",
                # variables = c('X1','`Group (A,B,C)`', 'Category'),
                variables = c('X1','Category'),
                ...
                init_method = 'weighted.median', # 'meta','median','weighted.median','lead'
                optim_method = 'BFGS',
                optim_maxit = 100,
                lead_site = 'site1',
...
pda::pda(site_id = 'site1', control = control, dir = mydir, upload_without_confirm = T,silent_message =F)
...
pda::pda(site_id = 'site3', ipdata = data_split[[3]], dir=mydir, upload_without_confirm = T)
  
S=readline(prompt="Type  <Return>   to continue : ")
## assume remote site2: enter "1" to allow tranferring your local estimate
pda::pda(site_id = 'site2', ipdata = data_split[[2]], dir=mydir, upload_without_confirm = T)

S=readline(prompt="Type  <Return>   to continue : ")
## assume lead site1: enter "1" to allow tranferring your local estimate
## control.json is also automatically updated
pda::pda(site_id = 'site1', ipdata = data_split[[1]], dir=mydir, upload_without_confirm = T)
...
config <- pda::getCloudConfig(site_id =control$lead_site, dir=mydir)
fit.pda = pda::pdaGet(name = 'site1_estimate', config = config)
sqrt(diag(solve(fit.pda$Htilde))) # inv-Hessian s.e. est
# 0.1101771 0.3178109 0.2988028 0.3016307 0.2845887
print("PDA")
print(fit.pda$btilde)
print("pooled")
print(beta_pooled$coef)
print("meta")
print(fit.pda$bbar)
